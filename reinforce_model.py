...
from min_simul_v5 import Simulator
...
class CustomEnv(gym.Env):
    ...
    def __init__(self, mode, start_time, fin_time, interval, init_balance):
        super(CustomEnv, self).__init__()
        # self.algo
        self.mode = mode
        self.init_balance = init_balance
        if self.mode == 0:
            s = Simulator(init_balance, 0)
            s.setting(setting=[1,0,0,1])
            s.init_total_transaction_data(start_time, fin_time, "btckrw")
            self.total_data = s.total_df.to_numpy()
        self.reward = init_balance
        self.observed_rows_num = interval
        self.execute_ratio_krw = 10000
        # Define action and observation space
        # They must be gym.spaces objects
        self.action_space = spaces.Discrete(3)
        dict_space = {
            "data": spaces.Box(low=-np.inf, high=np.inf, shape=(self.observed_rows_num, 5)),
            "worth": spaces.Box(low=-np.inf, high=np.inf, shape=(1, 3))
        }
        ...
        #
        self.observed_rows_num = 120 # default
        self.current_step = 0
        # buy / sell / stay

        # Worth data
        self.coin_worth = 0
        self.krw_worth = init_balance
        self.net_worth = init_balance
        # ticker data
        # simulation / train / evaluation with db datasets
    ...
    def step(self, action):
        ...
        if self.mode == 0:
            current_data = self.total_data[self.current_step:self.current_step+self.observed_rows_num]
            current_price = float(current_data[-1,3])
            coin_amount = self.execute_ratio_krw/current_price
            if self.trade == "buy":
                self.coin_worth += coin_amount * (1-0.25/100)
                self.krw_worth -= self.execute_ratio_krw
            if self.trade == "sell":
                self.coin_worth -= coin_amount
                self.krw_worth += self.execute_ratio_krw * (1-0.25/100)

            self.net_worth = self.coin_worth * current_price + self.krw_worth

        self.worth_obs = [self.krw_worth, self.coin_worth, self.net_worth]
        self.current_step += 1
        self.data_obs = self.total_data[self.current_step:self.current_step+self.observed_rows_num]
        obs = {
            "data": self.data_obs,
            "worth": self.worth_obs
        }

        done = self.current_step+self.observed_rows_num+1 == len(self.total_data)
    ...
    def reset(self):
        self.done = False
        self.current_step = 0
        self.krw_worth = self.init_balance
        self.coin_worth = 0
        self.net_worth = self.krw_worth
